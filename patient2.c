...
int main() {
	...
	if (strcmp(buf, "notavailable") == 0) {
		printf("Phase 2: The requested appointment from Patient 2 is not available.\n");
		close(fd);
		return 0;
	} else {
		token = strtok(buf, " ");
		...
		token = strtok(NULL, " ");
		docport = atoi(token);
		printf("Phase 2: The requested appointment is available and reserved to Patient 2. The assigned doctor port number is: %d.\n", docport);
		close(fd);
	}
	...
	printf("Phase 3: Patient 2 has a dynamic UDP port number %u and IP address %s.\n", ntohs(myaddr.sin_port), inet_ntoa(myaddr.sin_addr));
	...
	printf("Phase 3: The cost estimation request from Patient 2 with insurance plan %s has been sent to the doctor with port number %u and IP address %s.\n", buf, ntohs(servaddr.sin_port), inet_ntoa(servaddr.sin_addr));
